...
% There is not one metric universally best-suited to measuring the invariance (or fairness) of
% representations.
...
$f(\cdot )$, which is trained on the representation $z$, i.e.\  $f: Z \to \hat{Y}$.
...
representations are transferrable. Here, an adversary is used to remove sensitive information from
...
and coping with sampling bias - especially extreme cases where $|\, \textrm{supp}(S_{tr} \times
Y_{tr}) | < |\, \textrm{supp}(S_{te} \times Y_{te}) |$.
