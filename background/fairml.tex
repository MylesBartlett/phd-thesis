...
\marginpar{\textbf{Notions of fairness}}
%
...
interchangeably throughout the literature; I will favour the former).
%
I will denote group membership using the random variable \( S: \Omega \to \gS \) -- with
...
%
Particularly, I will consider two particular kinds of group-oriented notions of fairness: 1) the
...
Simply put, the unified goal of \ac{AF} is to learn some predictor, \(f: \gX \to \gY \) that
...
%
In invariant-representation learning (encompassing \ac{FRL}, \ac{DA}, and \ac{DG} and referred to
within the context of the former suitably as \ac{FRL}), for example, a common method for imparting
independence, \(Z \perp S \) -- which is sufficient for \(Y \perp S\), given a predictor head, \(g:
\gZ \to \gY \) -- between the representations learned by encoder \(h: \gX \to \gZ \) and the
sensitive attribute to train \(h\) to maximise the conditional entropy \( H(\hat{S}|Z) \) of an
adversarial predictor, \(a: \gZ \to \bigtriangleup^{|\gS|}\) (itself trained via \ac{MLE}).
%
...
%
It has long been recognised that there exists an inherent trade-off between the utility, as
...
distribution, the DRO objective defines a bilevel optimisation problem in which only the supremum
of the loss over \( \mathfrak{P}(X, Y) \) contributes to the overall objective to be minimised by
our predictor, \(f\).
%
...
%
In the \ac{DG}/group-robustness literature this quantity is commonly known as \ac{RobAcc}.
%
...
summed up with the apophthegm `similar individuals should receive similar treatments'.
%
As alluded to before, the premise of \ac{IF} greatly resembles the smoothness assumption from
SemiSL. 
%
I mentioned in its associated section that the smoothness assumption can be seen as a
...
% ------------------------------------------------------------------------------ 
I conclude this section with a brief discussion of the residual unfairness, as termed by
\cite{kallus2018residual}, and the problem setting giving rise to it, owing to its pertinence to
both Chapter~\ref{ch:nifr} and Chapter~\ref{ch:supmatch}.
%
