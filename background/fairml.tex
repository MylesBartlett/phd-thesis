...
the mutual information (MI) between \(Y\) and \(S\), defined as the Kullback-Leibler (KL)
...
\citep{martinez2020minimax} in Pareto-optimal terms, though the same idea has long existed in the
distributionally robust optimisation (DRO), and an idea which has been inherited by domain
generalisation as a offshoot of DRO. 
%
...
\begin{equation}
    ...
    \E_{(X, Y) \sim P(X, Y)}[D(\Gamma(X), Y) ],
 \end{equation}
%
recalling that we use \(D(\cdot, \cdot)\) to denote some statistical divergence.
%
