...
that is the minimiser of the eponymous \emph{risk}, $\gR$, defined according to some statistical
distance, or \emph{loss}, \( \gL: \R^\Omega \times \gY^\Omega \to \R \) between the predicted,
\(\hat{Y} \triangleq f(X) \), and ground-truth labels, \(Y\) over the training distribution,
\(P^{tr}(X,
Y)\). 
%
...
%
\begin{equation}\label{eq:pop_risk} \gR (f) \triangleq \E_{ (X, Y) \sim P^{tr}(X, Y) } [ L ( f(X,
    Y) )].
...
Despite its intuitiveness and long history, with roots in early statistical modelling, the
