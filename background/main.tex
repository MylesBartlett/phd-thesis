...
very least spirit-dampening to the those with lofty hopes for ML -- it is not surprising that this
SB and that SCL should arise widely because of it, for while SB alone is not alone a precondition
for SCL, the second precondition  of SCL is a problem (on the data side) that has long challenged
statistical modelling: sampling bias.
%
Indeed, it is the combination of simplicity bias and acausal or spurious, correlations generated by
...
%
Although the problem may stem from the data-collection side, that is not to say that there is
nothing that can be done on the modelling side, for both DG and FairML actively contend with
different flavours of the problem and have been successful in developing methodologies to
successfully mitigate them.



the myriad guises under which the phenomenon has appeared in the ML and literature and that of
adjacent field --
