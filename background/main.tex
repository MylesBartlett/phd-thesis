...
/\begin{enumerate}
  \item 
    ...
    simultaneously).
    %
    A factorisation, on the other hand, that does not accord with this can be said to be
    \emph{entangled}.
 ...
 A common \citep{arjovskyinvariant, krueger2021out, sagawa2019distributionally} and natural way of
 framing the robust-prediction problem is as one of bilevel optimisation, where the inner loop
 entails computing the empirical risk over each domain and the outer loop corresponds to finding
 the function that minimises the maximum of said risks.
 %
 According to this formulation, we can then couch the optimal predictor as 
 %
...
domains, \(\gE^\dagger\) outside \(\gE\) (the goal of DG), \(\gE\) must be a representative
(well-covering) set of samples from the underlying distribution \(\P(E)\) such that interpolation
is feasible.
