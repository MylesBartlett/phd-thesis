...
proposed an \ac{INN}-based transfer-learning approach -- transferring invariance from a
...
representative set featured in Chapter 3;
%
finally, in Chapter \ref{ch:okapi} we grappled with the problem of semi-supervised DG -- by which I
mean the problem of how to make effective use of unlabelled data drawn from extra domains to
...
%
Nonetheless, this discovery can fail to align with expectation, and we assume in Chapter
\ref{ch:supmatch} that the subgroups/targets are sufficiently salient to be well-clustered by
un-/semi-supervised means -- one would not expect a source formed from gender/pulmonary
...
%
I have spoken before of the practical deficiencies of the AdvL paradigm in a DL context -- namely
the fragility of optimisation, disposition to cyclical dynamics, architectural dependency, and the
loss of guarantees incurred by estimating the best-response dynamics with a finite (typically
...
setups.
%
We found the inherent instability of AdvL particularly pronounced in the case of Chapter
\ref{ch:nifr}, specifically, owing to the compounding instabilities imparted by the invertible
architectures, a problem which itself would not be addressed \emph{in toto} until
...
%
In Chapter \ref{ch:supmatch} we provided theoretical guarantees regarding when the proposed
...
both within the context of the various subfields ML concerned with it (DA, DG, AF) and within the
broader context of game theory, dynamical systems theory, and information theory -- however, we
again furnish only intuition and empirical claims for the stabilisation techniques, for which
(proof-driven) theoretical-grounding would be demonstrably desirable.
...
fuelling the eponymous Reinforcement Learning from Human Feedback (\ac{HF};
\cite{christiano2017deep, stiennon2020learning, bai2022training}), analogously termed \acf{RLAIF}.
%
...
%
% I have already spoken of foundation models \citep{bommasani2021opportunities}, albeit in the
% narrow context of AI-alignment, but 
...
% \citep{bai2022constitutional}): for text-based tasks, \ac{RLHF} has emerged as the leading paradigm
...
% \ac{RLHF} is annotation-intensive -- though this can be ameliorated with crowdsourcing --
% \cite{bai2022constitutional} showed that given an existing \ac{RLHF}-trained model, one can improve
% performance along the harmlessness axis (i.e. de-bias the model) based on the guidance of the
% \ac{RLHF} model itself, as given by some constitution (a set of values encoded in the form of
% prompts); this variant of \ac{RLHF} has been appropriately termed RLAIF (Reinforcement Learning from
% AI feedback).
% While \ac{RLHF} was initially conceived for, and has been almost-exclusively since applied to,
...
% \cite{lee2023aligning} in extending \ac{RLHF} to text-to-image generation.
%
...
% description) for its classification, rather than mere logits; \ac{RLHF}/RLAIF can then be conducted
