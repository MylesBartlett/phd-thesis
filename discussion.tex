...
\corr{
    %
    There is sound argument to made that that the datasets featured in Chapters~\ref{ch:nifr} and
    \ref{ch:supmatch}, namely Coloured MNIST (cMNIST) and CelebA, do not well represent performance
    on real-world problems, both due to their nature and their paucity.
    %
    ...
    characteristically simple -- dataset in MNIST \citep{lecun1994mnist}.
    %
    The latter consists of real-world images however their preprocessing (resizing to a relatively
    low resolution and tight cropping) means for a poor approximation for the kinds one would
    realistically encounter in the wild. 
    %
    In Chapter~\ref{ch:nifr}, it would be particularly interesting to extend the transfer-learning
    experiments presented in \ref{sec:transfer-learning} beyond the synthetic realm, using, for
    instance, the recently-introduced NICO++ \citep{zhang2023nicopp} dataset.
    %
    Ongoing work on Chapter~\ref{ch:supmatch}, that attempts to shore up the deficit discussed
    here, features this very dataset -- adapted to our problem setup -- along with a medical
    imaging dataset in Chest-Xray8 \citep{wang2017chestx} to bolster the motivation, healthcare
    being a sector particularly prone to the problem we bring to attention and formalise.
    %
    Chapter~\ref{ch:okapi}, in contrast, was elevated greatly by the datasets in question being
    part of an established benchmark suite (defining an established problem) assembled from
    pre-existing real-world datasets.
    %
}
