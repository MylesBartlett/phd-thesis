...
%
\section*{The thesis of stories}
%
\noindent It is well-acknowledged that every good story should have a beginning, a middle, and an
...
counterfeit or embellished; as a story, this is a work of chapters and bridging those chapters is
evolution, both academically and personally.
%
...
where each step moves us further along on a journey, one without a destination, but a journey one
should never yield on regardless of the times one stumbles. 
%
I am not ashamed to admit that the journey paved by this thesis itself was marked by many such
...
fears, but I can at least say that I forged on -- as the existence of this thesis, in its
completion, attests to -- not by own mettle alone but as much by the support and succour of those
who have staunchly companioned me each step of the way, through summer-sweetened meadow and
gloom-drenched thicket alike.
%
I would also call this thesis a \emph{story} for simply for my fanciful fondness of stories,
for all that they might teach us about others, and, most of all, ourselves -- by exploring fabulous
new worlds we also come to better understand the inner one -- and I would naturally like to like
something I have devoted so much to, whose lesser elements I might recast, like in some dualistic
tale, as the shadows that make the light -- the nobler qualities -- burn all the brighter. 
%
For while every lit candle casts a shadow, it is equally true that the darker, the more moonless
the night, the more splendent the starlight.
...
demons -- indeed, the theme is not quite so elevating -- having the power to rouse our
best-possible selves -- but it bears its own importance, nevertheless -- not to the human condition
but rather to the autonomous one.
...
voyage: though this theme is the first of in the title -- `semi-supervised methods for\dots' -- it
is the last I shall write of here.
%
...
%
This is, consequently, a shortcut-learning problem in which the former attribute serves as a proxy
...
nonetheless and there are undoubtedly cases where this is limiting.
%
In the second chapter, we consider a relaxed version of the problem -- couched
in a way that is not AF-centric -- which is soluble using supplementary data that is unannotated in
the truest sense, in that neither the sensitive -- here, `subgroup' -- nor target annotations are
provided.
%
The problem is still one of shortcut-learning, however, the biasing is imposed in an asymmetric
fashion such that the identifiability is possible; its general formulation -- which admits the
problem from the previous chapter as a special case -- is as much a contribution of the chapter as
the solution we ultimately propose to it, predicated on the idea of aligning support rather than
distributions as historically practised in domain adaptation \citep{ben2006analysis}.
%
