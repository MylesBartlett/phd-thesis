...
\begin{figure}[tbp]
  ...
  \caption{
    ...
    %
    Given the encoding, $\mbf{z}$, of anchor (query) image, \(\mbf{x}\), the corresponding domain label
    $s$, and propensity score $e$, \CNN{} yields the closest samples, according to distance $d$,
    subject to the constraint that those samples belong to a different a different domain to the
    anchor (cross-domain constraint), as well as the constraints imposed by the two calipers.
    %
    Red crosses denote exclusion of the keys from the pool of candidate-matches, such that the
    \(k\)-closest (\(k\) being \(1\) here for simplicity's sake and `closeness' determined by
    \(d\)) unmarked samples become the outputs of the matching algorithm.
    %
    Note that the, the matching is bipartite, such that self-matches are proscribed.
    %
    ...
    propensity scores, which is to say overly-characteristic of their respective domains, are
    removed from the pool of candidate-matches. 
    %
    In the corresponding panel above, the set of samples at this stage is singular, consisting of only the
    bottom-most image (key).
    %
    ...
    %
    In the corresponding panel above, exclusion by the std. caliper is denoted with a red cross
    over the top-most edge, leaving only two candidates keys, the top-most of which is implied to
    be closest by its selection in the final step.
  %
