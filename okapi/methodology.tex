...
\gX \to \mathbb{R}^d$, generating intermediary outputs (features) $z \triangleq
...
\end{align}
%
where $d: \mathbb{R}^d \times \mathbb{R}^d \to \mathbb{R}$ is some distance function.
%
...
classifier $f_\theta$, $h_\psi: \mathbb{R}^d \to \bigtriangleup^{|\gS|}$ where
...
$w_{\mbf{s}_k}: \gS \to \mathbb{R}^+_*$ is a function assigning to each $s$ an importance weight
