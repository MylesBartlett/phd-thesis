...
\gX \rightarrow \mathbb{R}^d$, generating intermediary outputs (features) $z \triangleq
...
classifier $f_\theta$, $h_\psi: \mathbb{R}^d \rightarrow \bigtriangleup^{|\gS|}$ where
...
$w_{\mbf{s}_k}: \gS \rightarrow \mathbb{R}^+_*$ is a function assigning to each $s$ an importance weight
