...
1\} \). 
%
Results shown in \figref{fig:big-adult-chart} indicate that while our model fails to surpass the
...
%
The former observation is a natural consequence of the \emph{accuracy-fairness trade-off}
\citep{kaplow1999conflict}, a trade-off that applies here, in the case of the Adult dataset,
due to the immanence of systemic bias -- in this case, the propensity of males to earn more (at
the time of the census data's collection) -- and thereby the induction of causal dependence
between ``gender'' and ``salary'', yet not to the image datasets where the analogous attributes
are statistically entangled but causally independent -- i.e. while females may be more likely
to be have blonde hair, the observations thereof are fundamentally independent (given an
individual's gender and nothing else, one can do no better than randomly guess their hair colour,
assuming the relevant intersectional groups are balanced). 
%
% CORRECTED: can you say why this is?
%
